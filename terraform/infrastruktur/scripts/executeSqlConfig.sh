...
az postgres flexible-server execute -n "${SERVERNAME}" -u "${USERNAME}" -p "${SECRET}" -d postgres -q "CREATE USER \"${DBUSER}\" WITH PASSWORD \"${USERSECRET}\";" --output table
az postgres flexible-server execute -n "${SERVERNAME}" -u "${USERNAME}" -p "${SECRET}" -d postgres -q "GRANT ALL PRIVILEGES ON DATABASE \"${STUDENTDB}\" TO \"${DBUSER}\";" --output table
